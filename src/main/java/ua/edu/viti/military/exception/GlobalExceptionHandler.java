...
public class GlobalExceptionHandler {
    ...
    // Обробка порушення унікальності в БД (409 Conflict)
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(
        ...
        String message = "Порушення унікальності даних";
        
        // Спробуємо витягти яке поле дублюється
        if (ex.getMessage() != null) {
            if (ex.getMessage().contains("license_number")) {
                message = "Водій з таким номером посвідчення вже існує";
            } else if (ex.getMessage().contains("military_id")) {
                message = "Водій з таким військовим ID вже існує";
            } else if (ex.getMessage().contains("registration_number")) {
                message = "Транспорт з таким реєстраційним номером вже існує";
            } else if (ex.getMessage().contains("name")) {
                message = "Запис з такою назвою вже існує";
            }
        }

        ErrorResponse error = new ErrorResponse();
        error.setType("/errors/duplicate-data");
